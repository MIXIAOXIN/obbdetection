...
from mmdet.core import arb2result, arb2result_attr, arb2roi, build_assigner, build_sampler
...
class OBBAStandardRoIHead(OBBBaseRoIHead, OBBoxTestMixin):
    ...
    def forward_train(self,
        ...
        if self.with_bbox:
            ...
            for i in range(num_imgs):
                ...
                # print('proposal list size: ', proposal_list[i].shape)
                # print('ground truth size: ', target_bboxes[i].shape)
                # print('bbox assign result: ', assign_result)
                # print('bbox assign result, gt_inds', assign_result.gt_inds)
                # print('gt_masks: ', gt_masks)
                pos_gt_masks_data = torch.tensor(gt_masks[i].data.masks, dtype=torch.float64, device=target_bboxes[i].device)
                pos_gt_masks_data = pos_gt_masks_data.view(pos_gt_masks_data.shape[0], pos_gt_masks_data.shape[-1])

                sampling_result = self.bbox_sampler.sample_attr(
                    assign_result,
                    proposal_list[i],
                    target_bboxes[i],
                    gt_labels[i],
                    gt_masks=pos_gt_masks_data,
    ...
    def _bbox_forward(self, x, rois):
        ...
        bbox_results = dict(
            cls_score=cls_score, bbox_pred=bbox_pred, attr_score=attr_score, bbox_feats=bbox_feats)
        return bbox_results
    ...
    def simple_test_bboxes(self,
        ...
        # print('detected attrs: ', det_attrs)
