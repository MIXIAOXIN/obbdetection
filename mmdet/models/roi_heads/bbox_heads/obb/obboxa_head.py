...
from mmdet.core import (auto_fp16, build_bbox_coder, force_fp32, multi_apply,
                        multiclass_arb_nms, get_bbox_dim, bbox2type, arb2roi, multiclass_arb_nms_attr)
from mmdet.models.builder import HEADS, build_loss
from mmdet.models.losses import accuracy


@HEADS.register_module()
class OBBoxAHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.use_sigmoid_attr = loss_attr.get('use_sigmoid', False)
        ...
        if self.with_attr:
            self.fc_attr = nn.Linear(in_channels, num_attr)  ### ???? 不需要加1吗？一共有两个属性，0 or 1
    ...
    # 根据预测出的cls_score, bbox_pred, attr_score 等参数，选出bbox
    # cls_score 根据softmax选择
    # bbox 通过decode、rescale等操作，通过nms选择
    # attr_score 经过sigmoid 函数后，将其映射到0或1
    ...
    def get_bboxes(self,
        ...
        attrs = torch.sigmoid(attr_score)
        attrs = attrs.ge(0.5)
        ...
        if cfg is None:
            ...
            det_bboxes, det_labels, det_attrs = \
                    multiclass_arb_nms_attr(bboxes, scores, attrs, cfg.score_thr,
